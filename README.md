...
`.env` 中的 `DB_ENGINE=mysql`、数据库地址和账号应与本机配置一致。测试环境仍使用独立SQLite文件，不影响MySQL数据。
...
当前正式开发配置使用本机 MySQL；`schema.sql` 会创建用户、病例、健康记录、分析结果、体质结果和知识框图表。没有MySQL时可把 `DB_ENGINE` 临时设为 `sqlite` 进行接口测试，但不应把SQLite作为正式数据存储。
